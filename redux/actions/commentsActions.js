...
modifyComment: (comment, commentId) => {
    ...
    deleteComment: (id) => {
        ...
        const token = localStorage.getItem('token')
        return async (dispatch, getState) =>{
            const res = await axios.post(`https://mytinerary-amuchastegui-rodrig.herokuapp.com/api/itineraries/comment/${id}`, {}, { //Le paso datos por params(id) id del comentario que quiero eliminar
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            })
